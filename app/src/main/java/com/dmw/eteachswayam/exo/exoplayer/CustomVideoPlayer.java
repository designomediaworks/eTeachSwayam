package com.dmw.eteachswayam.exo.exoplayer;
...
public class CustomVideoPlayer extends  LinearLayout {
    ...
    private void prepareVideoPlayer() {
        if (urlIndex < urlList.size()) {
            exoPlayer.prepare(buildMediaStore(urlList.get(urlIndex)), true, true); //http url
        // exoPlayer.prepare ( buildMediaStorelocal ( urlList.get ( urlIndex ) ),true,true );
    ...
    private MediaSource buildMediaStorelocal(String mediaUrl) {
        MediaSource videoSource1 = null;
        try {

            Uri                   videoUri        = Uri.parse ( mediaUrl );
            DefaultBandwidthMeter bandwidthMeterA = new DefaultBandwidthMeter ( );
            //DefaultDataSourceFactory        dataSourceFactory               = new DefaultDataSourceFactory ( this , Util.getUserAgent ( context , context.getPackageName () ) , bandwidthMeterA );
            DefaultDataSourceFactory        dataSourceFactory               = new DefaultDataSourceFactory ( context , "custom_video_player" , bandwidthMeterA );
            DynamicConcatenatingMediaSource dynamicConcatenatingMediaSource = new DynamicConcatenatingMediaSource ( );
            ExtractorsFactory               extractorsFactory               = new DefaultExtractorsFactory ( );
            videoSource1         = new ExtractorMediaSource ( videoUri , dataSourceFactory , extractorsFactory , null , null );
